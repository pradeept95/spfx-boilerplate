...
export default function useDataTable<T extends object>(props: Partial<IDatagridType<any>>) {
  ...
  React.useEffect(() => {
    prepareGridColumns();
  }, [props.columns, filteredItems])


  const initializeItemsAndPagedItems = () => {
    setAllItems(props?.items);
    setFilteredItems(props?.items);
    setPagedItems(props?.items?.slice(0, 10));
  }
  ...
  const prepareGridColumns = () => {
    const newColumns = props?.columns?.map(col => {
      return {
        ...col,
        onColumnClick: (e, c) => _onColumnClick(e, c, props?.columns, filteredItems)        
      } as IColumn;
    });
  ...
  const onColumnSort = (column: IColumn, allColumns: IColumn[], itemsToSort: T[]) => {
    ...
    newColumns.forEach((newCol: IColumn) => {
      ...
      newCol.onColumnClick = (e, c) => _onColumnClick(e, c, props?.columns, itemsToSort)

    });
    const newAllSortedItems = _copyAndSort(itemsToSort, currColumn.fieldName!, currColumn.isSortedDescending);
    ...
    updatePagedItemData(newAllSortedItems, 1, 10)
  }
  ...
  const onFilterTable = (searchText: string) => {
    ...
    const currentFilteredResult = allItems?.filter(function (item) {
      return Object.keys(item).some(function (k) {
        return (item as any)?.[k]?.toString()?.toLowerCase()?.indexOf(`${searchText}`?.toLocaleLowerCase()) != -1;
      })
    })

    // reset page to first page and update pagedData 
    setFilteredItems(currentFilteredResult)
    updatePagedItemData(currentFilteredResult, 1, 10)
  }

  const updatePagedItemData = (items: T[], currentPage: number, pageSize: number) => {
  ...
  function _onColumnClick(ev: React.MouseEvent<HTMLElement>, column: IColumn, allColumns: IColumn[], itemsToSort: T[]): void {
    onColumnSort(column, allColumns, itemsToSort)
  };

  return [pagedItems, filteredItems, currentPage, columns, onPageChange, onFilterTable] as const;
}
